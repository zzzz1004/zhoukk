...
let postPetMsg = (req, res) => {  
	...
	db.is_pet_exists('select * from dangAnMsg where petname=? and petusername=?', [petname, petusername], (result) => {
		num = result.length
		if(num === 0){
			sql = 'INSERT INTO dangAnMsg (petname, phone, history, petusername) VALUES (?, ?, ?, ?)';
		}
		db.exec(sql, params, (data) => {  
			// 确保这里使用正确的返回属性  
